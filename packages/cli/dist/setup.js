...
async function main() {
    ...
    console.log('\n🤖 Step 2b: Bot Identity\n');
    const identityResponse = await (0, prompts_1.default)([
        {
            type: 'text',
            name: 'botName',
            message: 'Bot Name (max 32 chars):',
            initial: overrides.botName || '',
            validate: (v) => v.length > 0 && v.length <= 32 ? true : 'Name must be 1-32 characters',
        },
        {
            type: 'select',
            name: 'category',
    ...
    if (network === 'devnet') {
        ...
        try {
            ...
            if (balance < REQUIRED_SOL * web3_js_1.LAMPORTS_PER_SOL) {
                // Request multiple airdrops (devnet limits ~2 SOL per request)
                const rounds = Math.ceil((REQUIRED_SOL * web3_js_1.LAMPORTS_PER_SOL - balance) / (2 * web3_js_1.LAMPORTS_PER_SOL));
                for (let i = 0; i < Math.min(rounds, 6); i++) {
                    try {
                        console.log(`  💧 Airdrop ${i + 1}/${rounds}...`);
                        const sig = await connection.requestAirdrop(kp.publicKey, 2 * web3_js_1.LAMPORTS_PER_SOL);
    ...
    const programId = new web3_js_1.PublicKey(DJINN_PROGRAM_ID);
    try {
        const balance = await connection.getBalance(kp.publicKey);
        if (balance < 10 * web3_js_1.LAMPORTS_PER_SOL) {
            console.log(`  ⚠️  Insufficient balance (${balance / web3_js_1.LAMPORTS_PER_SOL} SOL). Need 10+ SOL.`);
            console.log('  Skipping on-chain registration. You can register later via the web.\n');
        }
        else {
            // Derive PDAs
            const [botProfilePDA] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('bot_profile'), kp.publicKey.toBuffer()], programId);
            const [botEscrowPDA] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('bot_escrow'), kp.publicKey.toBuffer()], programId);
            ...
            if (existingAccount) {
                ...
                const discriminator = (0, crypto_1.createHash)('sha256')
        ...
        if (msg.includes('custom program error: 0x0'))
            msg = 'Bot already registered or name taken';
